...
def insert_film_works_and_directors():
    ...
    insert_batch_pg(
        "INSERT INTO content.film_work (id, title, description, rating, old_id) VALUES (%s, %s, %s, %s, %s)",
        film_work,
    )
    insert_batch_pg("INSERT INTO content.person (id, name) VALUES (%s, %s)", person)
    insert_batch_pg(
        "INSERT INTO content.person_film_work (id, film_work_id, person_id, role) VALUES (%s, %s, %s, %s)",
        person_film_works,
    )
...
def insert_genre_and_genre_film_work():
    ...
    insert_batch_pg(
        "INSERT INTO content.genre (id, name, old_id) VALUES (%s, %s, %s)", genres
    )
    ...
    for movie_genre_data in movie_genres_data:
        ...
        pg_film_work_id = get_data_from_pg(
            f"SELECT id FROM content.film_work WHERE old_id = '{sqlite_movie_id}'"
        )[0]["id"]
        sqlite_genre_id = movie_genre_data["genre_id"]
        pg_genre_id = get_data_from_pg(
            f"SELECT id FROM content.genre WHERE old_id = '{sqlite_genre_id}'"
        )[0]["id"]
        genre_film_work.append((str(uuid.uuid4()), pg_film_work_id, pg_genre_id))

    insert_batch_pg(
        "INSERT INTO content.genre_film_work (id, film_work_id, genre_id) VALUES (%s, %s, %s)",
        genre_film_work,
    )
...
def insert_actors_into_person_and_person_film_work():
    ...
    for actor in actors_data:
        ...
        person_exists = get_data_from_pg(
            f"SELECT * FROM content.person WHERE name = '{query_name}'"
        )
        if not person_exists:
            actors.append((str(uuid.uuid4()), actor["name"]))
    insert_batch_pg("INSERT INTO content.person (id, name) VALUES (%s, %s)", actors)
    ...
    for movie_actor_data in movie_actors_data:
        ...
        pg_actor_id = get_data_from_pg(
            f"SELECT id FROM content.person WHERE name = '{sqlite_actor_name}'"
        )
...
def insert_writers_into_person_and_person_film_work():
    ...
    insert_batch_pg("INSERT INTO content.person (id, name) VALUES (%s, %s)", writers)
    ...
    for movie_writer_data in movie_writers_data:
        ...
        pg_writer_id = get_data_from_pg(
            f"SELECT id FROM content.person WHERE name = '{sqlite_writer_name}'"
        )
...
def drop_old_id_columns():
    alter_pg("ALTER TABLE content.film_work DROP COLUMN old_id")
    alter_pg("ALTER TABLE content.genre DROP COLUMN old_id")


def tests():
    director_lucas = get_data_from_pg(
        """SELECT pf.role, COUNT(f.title) FROM content.person p INNER JOIN content.person_film_work pf ON p.id = pf.person_id
        INNER JOIN content.film_work f ON f.id = pf.film_work_id where p.name ='George Lucas' GROUP BY pf.role"""
    )
    print(
    ...
    news_genre = get_data_from_pg(
        """SELECT f.title FROM content.film_work f INNER JOIN content.genre_film_work gf ON f.id = gf.film_work_id 
        INNER JOIN content.genre g ON gf.genre_id = g.id WHERE g.name='News'"""
    )
